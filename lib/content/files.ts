...
export function parseLocalizedContentPath(baseDir: string, filePath: string): {
  slug: string;
  locale: AppLocale;
} {
  const relative = path.relative(baseDir, filePath).replace(/\\/g, '/').replace(/\.mdx?$/, '');
...
export function slugToFileCandidates(baseDir: string, slug: string, locale: AppLocale): string[] {
